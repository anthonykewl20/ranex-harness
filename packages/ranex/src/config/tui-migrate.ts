...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from ranex.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "ranex"),
    ...(await Filesystem.findUp(["ranex.json", "ranex.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ranex"))
  }
  if (Flag.RANEX_CONFIG) files.push(Flag.RANEX_CONFIG)
