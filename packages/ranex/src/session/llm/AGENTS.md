...
- `ai-sdk.ts` converts AI SDK `fullStream` parts into `@ranex/llm` `LLMEvent`s. This is the default runtime path.
- `native-request.ts` converts opencode's normalized session input into a native `@ranex/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    ai-sdk.ts               AI SDK fullStream -> @ranex/llm LLMEvent adapter
    native-request.ts       opencode/AI SDK-shaped input -> @ranex/llm LLMRequest
    native-runtime.ts       native runtime gate, tool bridge, and LLMClient handoff
```

Integration points:

- `../llm.ts` imports `LLMClient` from `@ranex/llm/route`; native execution is the only path that calls it directly.
...
- `native-request.ts` is the only adapter file that should construct `LLM.request(...)`, `LLM.model(...)`, `Message.*`, `SystemPart`, `ToolCallPart`, `ToolResultPart`, or `ToolDefinition` values from `@ranex/llm`.
- `ai-sdk.ts` and `native-runtime.ts` both emit `@ranex/llm` `LLMEvent`s so downstream session processing does not care which runtime handled the request.
...
`native-runtime.ts` evaluates the gate and either bridges into `@ranex/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays opencode-owned in both branches; only request lowering and transport differ.

Safety boundary:

- AI SDK remains the default.
- `RANEX_EXPERIMENTAL_NATIVE_LLM=true` or the umbrella `RANEX_EXPERIMENTAL=true` opts in. Native is not a global replacement.
