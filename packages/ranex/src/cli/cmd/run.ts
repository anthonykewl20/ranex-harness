import type { PermissionV1 } from "@ranex/core/v1/permission"
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@ranex/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run ranex with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running ranex server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to RANEX_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to RANEX_SERVER_USERNAME or 'ranex')",
      })
      .option("dir", {
        type: "string",
