...
import { EventSequenceTable } from "@ranex/core/event/sql"
...
import { WorkspaceTable } from "@ranex/core/control-plane/workspace.sql"
...
const originalEnv = {
  RANEX_AUTH_CONTENT: process.env.RANEX_AUTH_CONTENT,
  RANEX_EXPERIMENTAL_WORKSPACES: process.env.RANEX_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.RANEX_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.RANEX_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.RANEX_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.RANEX_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.RANEX_EXPERIMENTAL_WORKSPACES).toBe("true")
