...
import { resolve, type Info, type Resolved } from "@ranex/tui/config"
...
export function mockTuiRuntime(dir: string, plugin: PluginSpec[], opts?: { plugin_enabled?: Record<string, boolean> }) {
  process.env.RANEX_PLUGIN_META_FILE = path.join(dir, "plugin-meta.json")
  ...
  return {
    ...
    restore: () => {
      cwd.mockRestore()
      wait.mockRestore()
      delete process.env.RANEX_PLUGIN_META_FILE
    },
  }
}
