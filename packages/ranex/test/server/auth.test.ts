...
describe("ServerAuth", () => {
  test("does not emit auth headers without a password", () => {
    ...
    Flag.RANEX_SERVER_USERNAME = "alice"

    expect(ServerAuth.header()).toBeUndefined()
    expect(ServerAuth.headers()).toBeUndefined()
  })

  test("defaults to the ranex username", () => {
    Flag.RANEX_SERVER_PASSWORD = "secret"
    Flag.RANEX_SERVER_USERNAME = undefined

    expect(ServerAuth.headers()).toEqual({
      Authorization: `Basic ${Buffer.from("ranex:secret").toString("base64")}`,
    })
  })
  ...
  test("validates decoded credentials against effect config", () => {
    ...
    expect(ServerAuth.authorized({ username: "ranex", password: Redacted.make("secret") }, config)).toBe(false)
  })
})
