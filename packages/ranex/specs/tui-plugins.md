...
- Author package entrypoint is `@ranex/plugin/tui`.
...
- Import types from `@ranex/plugin/tui`.
- `@ranex/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `RANEX_CONFIG_DIR`, OpenCode installs `@ranex/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@ranex/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@ranex/plugin/tui`.
...
- `--pure` / `RANEX_PURE` skips external TUI plugins only.
