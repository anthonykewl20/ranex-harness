...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ranex.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ranex.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ranex.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ranex.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ranex.json", "ranex.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
