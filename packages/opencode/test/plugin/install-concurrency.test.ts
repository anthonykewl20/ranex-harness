...
describe("plugin.install.concurrent", () => {
  test("serializes concurrent server config updates across processes", async () => {
    ...
    const cfg = await read(path.join(tmp.path, ".opencode", "ranex.jsonc"))
  ...
  test("serializes concurrent server+tui config updates across processes", async () => {
    ...
    const server = await read(path.join(tmp.path, ".opencode", "ranex.jsonc"))
  ...
  test("preserves updates when existing config uses .json", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".opencode", "ranex.json")
    ...
    expect(await Filesystem.exists(path.join(tmp.path, ".opencode", "ranex.jsonc"))).toBe(false)
  }, 25_000)
})
