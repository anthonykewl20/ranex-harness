...
//   - RANEX_CONFIG_CONTENT      : provider config inline, no files to find
//   - RANEX_TEST_HOME           : pins os.homedir() → tmpdir
//   - RANEX_DISABLE_PROJECT_CONFIG : skip walking up for ranex.json
//   - RANEX_PURE                : skip external plugin discovery + install
//   - RANEX_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    RANEX_CONFIG_CONTENT: configJson,
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          RANEX_CONFIG_CONTENT: JSON.stringify({
