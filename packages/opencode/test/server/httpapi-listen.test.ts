...
const auth = { username: "ranex", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.RANEX_SERVER_PASSWORD
  else process.env.RANEX_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.RANEX_SERVER_USERNAME
  else process.env.RANEX_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.RANEX_SERVER_PASSWORD = auth.password
  Flag.RANEX_SERVER_USERNAME = auth.username
  process.env.RANEX_SERVER_PASSWORD = auth.password
  process.env.RANEX_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.RANEX_SERVER_USERNAME = auth.username
  delete process.env.RANEX_SERVER_PASSWORD
  process.env.RANEX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.RANEX_DISABLE_DEFAULT_PLUGINS
    process.env.RANEX_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.RANEX_DISABLE_DEFAULT_PLUGINS
      else process.env.RANEX_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
