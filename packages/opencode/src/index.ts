...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    if (opts.printLogs) process.env.RANEX_PRINT_LOGS = "1"
    if (opts.logLevel) process.env.RANEX_LOG_LEVEL = opts.logLevel
    if (opts.pure) {
      process.env.RANEX_PURE = "1"
    }
    ...
    process.env.AGENT = "1"
    process.env.RANEX = "1"
    process.env.RANEX_PID = String(process.pid)
  })
  .usage("")
