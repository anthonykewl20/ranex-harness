...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "ranex.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "ranex.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "ranex.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
  ...
  it.live("accepts $schema metadata without writing it into config files", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "ranex.json")
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "ranex.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "ranex.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "ranex.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "ranex.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "ranex.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "ranex.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "ranex.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".opencode", "ranex.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".opencode", "ranex.jsonc"),
