...
import { EffectDrizzleSqlite } from "@ranex/effect-drizzle-sqlite"
import { Effect, Layer } from "effect"
import { eq, inArray, sql } from "drizzle-orm"
import { DatabaseMigration } from "@ranex/core/database/migration"
import { migrations } from "@ranex/core/database/migration.gen"
import sessionUsageMigration from "@ranex/core/database/migration/20260510033149_session_usage"
import normalizeStoragePathsMigration from "@ranex/core/database/migration/20260601010001_normalize_storage_paths"
import sessionMessageProjectionOrderMigration from "@ranex/core/database/migration/20260603040000_session_message_projection_order"
import eventSourcedSessionInputMigration from "@ranex/core/database/migration/20260604172448_event_sourced_session_input"
...
import { SessionSchema } from "@ranex/core/session/schema"
import { SessionTable } from "@ranex/core/session/sql"
import sessionMetadataMigration from "@ranex/core/database/migration/20260511173437_session-metadata"
  ...
  test("applies tracked migrations to an empty database", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(
          yield* db.all(
            sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('permission_request', 'question_request') ORDER BY name`,
          ),
        ).toEqual([{ name: "permission_request" }, { name: "question_request" }])
        expect(
