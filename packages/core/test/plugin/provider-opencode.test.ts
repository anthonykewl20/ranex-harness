...
import { PluginHost } from "@ranex/core/plugin/host"
import { OpencodePlugin } from "@ranex/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ RANEX_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses RANEX_API_KEY as credentials", () =>
    withEnv({ RANEX_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ RANEX_API_KEY: undefined, CUSTOM_RANEX_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_RANEX_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
