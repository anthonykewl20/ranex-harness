...
import type { IntegrationOAuthMethodRegistration } from "@ranex/plugin/v2/effect/integration"
import { define } from "@ranex/plugin/v2/effect/plugin"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      const hasKey = Boolean(process.env.RANEX_API_KEY || connected || item.provider.request.body.apiKey)
