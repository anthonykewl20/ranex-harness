...
export type Decision =
  | { readonly _tag: "Idle" }
  | { readonly _tag: "ContinueCommitted"; readonly after: number }
  | { readonly _tag: "InterruptAmbiguousTool"; readonly assistantMessageID: SessionMessage.ID; readonly callID: string }
...
export function classify(input: {
  ...
  if (assistant) {
    ...
    if (running) return { _tag: "InterruptAmbiguousTool", assistantMessageID: assistant.id, callID: running.id }
    if (assistant.content.some((content) => content.type === "tool"))
      return { _tag: "ContinueCommitted", after: input.latestSeq }
