...
if [ -d "$RANEX_SRC/.git" ]; then
  ...
  git -C "$RANEX_SRC" checkout "$FORK_BRANCH"
  git -C "$RANEX_SRC" reset --hard "origin/$FORK_BRANCH"
else
...
PROMPT_FILE="$RANEX_SRC/packages/opencode/src/cli/cmd/tui/component/prompt/index.tsx"
...
cd "$RANEX_SRC/packages/opencode"
...
BUILT_BINARY="$RANEX_SRC/packages/opencode/dist/opencode-${PLATFORM}-${ARCH}/bin/opencode"

if [ ! -f "$BUILT_BINARY" ]; then
  BUILT_BINARY=$(find "$RANEX_SRC/packages/opencode/dist" -name "opencode" -type f -executable 2>/dev/null | head -1)
fi
...
if [ -f "$BUILT_BINARY" ]; then
  ...
  echo "  cd $RANEX_SRC/packages/opencode && bun run build --single"
  exit 1
fi
