...
- Do not replace private `packages/opencode` imports with `@ranex/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@ranex/tui`.
...
Status: Completed. The private `@ranex/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@ranex/tui`.
...
- The package has no dependency on `opencode`, `@ranex/cli`, or
  `@ranex/core`.
...
primitives now live in `@ranex/tui`. The legacy host consumes explicit
...
and build inputs through `@ranex/tui/runtime`. Movable app, component,
...
`@ranex/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@ranex/tui`. Configured references resolve through the new
...
`@ranex/tui`. The legacy host injects a narrow plugin host that retains
...
public `@ranex/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@ranex/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@ranex/core`, or either
  executable package.
...
rg '@ranex/core|packages/opencode|packages/cli' packages/tui
