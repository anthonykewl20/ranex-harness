...
Ambient project discovery canonicalizes and contains traversal within the project root and honors `RANEX_DISABLE_PROJECT_CONFIG`. An unavailable observation preserves the previously admitted value. A confirmed partial instruction removal emits the complete remaining aggregate with explicit supersession text; removing the final instruction emits a revocation message.
...
SLICE-012 (issue #2) bounds each provider stream with two distinct timeouts so a stalled or overrunning turn reaches a terminal run state within a bounded budget without manual interrupt: a per-pull **idle** deadline that resets on every emitted chunk and fires when the provider goes quiet mid-stream, and a whole-turn **absolute** budget that races the turn against a clock and interrupts the blocked consumer when it overruns regardless of chunk activity. Both fail as the same typed `LLMError` the existing failure-handling path already maps to a terminal assistant-error state, so no new error channel is introduced; each timeout is carried by its own distinct, non-retryable reason, so idle and absolute are distinguishable programmatically (not only by message text) and neither is retried back into the same stall. The two budgets are configurable and bounded through the harness configuration surface — a non-default value observably changes when the watchdog fires, an out-of-bounds value is refused at load, and the defaults are justified against real provider latency (including time-to-first-token) rather than hardcoded per provider, and a healthy slow stream is not falsely cut by either timeout. A stall with a tool call in flight reaches the same terminal state with the dispatched tool fiber terminated and the tool recorded interrupted. **Status update (2026-08-06):** the durable-execution program (ADR-015 in the ranex repo, milestone #1 on this repo) makes a provider watchdog — inactivity plus absolute timeout — the first deliverable, prototype-gated red-first before production. **Status update (2026-08-07):** the prototype gate is passed. A disposable worktree proved the watchdog claim red-to-green: a stalled provider stream hangs past budget without it, terminates within budget with it, still hangs with it disabled, and a healthy slow stream is not falsely cut. Idle and absolute timeouts are demonstrated independently — one scenario feeds deltas shorter than the idle threshold so only the absolute budget can fire. At that date, the policy design named in the first sentence remained the production slice's work (issue #2). **Status update (2026-08-07, SLICE-012 production):** the design above is what SLICE-012 (issue #2) builds in production — the first of ADR-015's five claims and the only one needing no schema change, amended before any code was written so the timeout reasons are non-retryable and idle and absolute are distinguishable by reason. Four implementation terminals work the nine done-criteria red-first in this repository; none had landed at the time. **Status update (2026-08-13):** the production watchdog shipped via SLICE-012 (`23d6a5b4ee`); a stalled run reaches a terminal state within budget and clears `sessions.active()`.
...
Configured cross-model failover is also fail-closed: a crash during fallback leaves `provider_in_flight` ambiguity rather than automatically restarting the configured fallback model, preserving at-most-once provider dispatch.

**Status update (2026-08-16):** The post-crash-continuation deferral below is superseded for single-host recovery. Recovery is classifier-driven and fail-closed: before each `llm.stream(request)` the runner durably starts the assistant step, so a crash after dispatch and before the first provider chunk is visible as provider ambiguity. Under the Session-ID flock recovery conditionally claims `session.execution_owner` and the EventV2 aggregate writer only for its write, releasing both afterward. Active blockers win; only an unsettled called (`running`) tool records one `Tool.Failed("Tool execution interrupted")` and a `tool_side_effect_ambiguous` blocker; a pending tool input and a provider turn without terminal evidence record `provider_in_flight` and are never retried automatically; a persisted retry settles its pre-output dispatch marker and waits for its recorded time; and only a fully settled tool turn may request one advisory continuation from reloaded history. Blockers carry only Session, assistant-message, call, and aggregate-sequence references. The core facade records an explicit `abandon` or `continue` choice with the resolving actor; that disposition does not itself dispatch work. Repeated sweeps, wakes, and takeovers converge without replaying tools or ambiguous providers. This recovery takeover is Linux-only: it relies on `/proc` boot and process-start identity. On platforms without that evidence, ownership is conservatively treated as live and recovery cannot converge; it fails closed, preserving at-most-once behavior. ADR-015 remains unchanged: no durable drain identity, execution lease, heartbeat, or tool-attempt replay record is introduced.

Post-crash continuation recovery is intentionally deferred. A wake does not infer that ambiguous provider work is safe to retry after an input has already been promoted. Explicit `run` may deliberately continue from durable projected history. A future recovery slice should model provider-dispatch ambiguity, required continuation, queued-input promotion, retry policy, and visible recovery status together. It must not assume an enclosing durable execution identity that the Session model does not otherwise need. **Status update (2026-08-06):** ADR-015 (ranex repo) fixes the reconciler gap — the eligible-input guard at the top of the runner returns before interrupted-tool reconciliation — and adds durable retry by publishing the existing `session.next.retried` event, plus Session-ID fencing via the existing flock and EventV2 owner claims. At-most-once with explicit interruption is the contract; no durable drain identity is introduced. **Status update (2026-08-07):** the prototype gate is passed for all four of those claims, in disposable worktrees, red-first with negative controls. The reconciler gap reproduces exactly as described — a crash with an empty inbox leaves a tool projected `running`, and only an empty inbox triggers it — and the hoist closes it idempotently. Durable retry required more than uncommenting the projector: `session.next.retried` was a no-op in the message updater and no attempt column existed, so the prototype added a session-level projection over the durable event rather than a counter table. Fencing reproduced the two-process double-drain first, then refused it. **Status update (2026-08-08):** SLICE-013 ships the reconciler gap in this repository. `failInterruptedTools` is hoisted above the eligible-input guard so an empty-inbox `run()` still reconciles, a `reconcile` capability exists on `SessionRunner`, and a startup sweep (`SessionReconcile.sweepNode`, wired into the application graph) reconciles stranded tools across all sessions at process boot with no `run()` call. Concurrent `reconcile()`/`run()` is serialized by a per-session mutex. The sweep is DB-global and therefore unsafe under concurrent processes: a second process booting will mark tools a first live process is actively running as interrupted. SLICE-011 reproduced that two-process case, so the exposure is demonstrated rather than assumed; it is accepted because the harness runs one daemon in normal operation, and the fencing slice must gate the sweep on a durable owner claim before more than one process is run against one database. **Status update (2026-08-13):** durable retry (SLICE-014, `2a098d4963`), durable blockers (SLICE-015, `0aea8a19a7`), and Session-ID fencing (SLICE-016, `1834c96260`) all shipped. SLICE-016 closed the SLICE-013 carry-forward: the startup sweep holds the per-session `EffectFlock`, the drain owns `execution_owner` through an atomic CAS at the `execution/local.ts` boundary, and a second live process is refused. Cross-process reclaim after a kill is bounded by the flock's approximately 60-second stale threshold (process-crash only, `synchronous = NORMAL`). It also fixed the production SQLite startup race: `PRAGMA busy_timeout` now runs before `PRAGMA journal_mode = WAL` in both native drivers (`sqlite.bun.ts`, `sqlite.node.ts`) and `database.ts`.
