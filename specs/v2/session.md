...
**Status update (2026-08-16):** The post-crash-continuation deferral below is superseded for single-host recovery. Recovery is classifier-driven and fail-closed: under the Session-ID flock it conditionally claims `session.execution_owner` and the EventV2 aggregate writer before acting. Active blockers win; an unsettled called tool records one `Tool.Failed("Tool execution interrupted")` and a `tool_side_effect_ambiguous` blocker; a provider turn without terminal evidence records `provider_in_flight` and is never retried automatically; a persisted retry waits for its recorded time; and only a fully settled tool turn may request one advisory continuation from reloaded history. Blockers carry only Session, assistant-message, call, and aggregate-sequence references, and explicit resolution records actor and resolution durably. Repeated sweeps, wakes, and takeovers converge without replaying tools or ambiguous providers. ADR-015 remains unchanged: no durable drain identity, execution lease, heartbeat, or tool-attempt replay record is introduced.
