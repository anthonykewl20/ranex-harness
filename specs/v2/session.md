...
**Status update (2026-08-16):** The post-crash-continuation deferral below is superseded for single-host recovery. Recovery is classifier-driven and fail-closed: before each `llm.stream(request)` the runner durably starts the assistant step, so a crash after dispatch and before the first provider chunk is visible as provider ambiguity. Under the Session-ID flock recovery conditionally claims `session.execution_owner` and the EventV2 aggregate writer only for its write, releasing both afterward. Active blockers win; only an unsettled called (`running`) tool records one `Tool.Failed("Tool execution interrupted")` and a `tool_side_effect_ambiguous` blocker; a pending tool input and a provider turn without terminal evidence record `provider_in_flight` and are never retried automatically; a persisted retry waits for its recorded time; and only a fully settled tool turn may request one advisory continuation from reloaded history. Blockers carry only Session, assistant-message, call, and aggregate-sequence references. The core facade records an explicit `abandon` or `continue` choice with the resolving actor; that disposition does not itself dispatch work. Repeated sweeps, wakes, and takeovers converge without replaying tools or ambiguous providers. ADR-015 remains unchanged: no durable drain identity, execution lease, heartbeat, or tool-attempt replay record is introduced.
