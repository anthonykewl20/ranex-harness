...
V2 core discovers config documents named `ranex.json` or `ranex.jsonc` in the global config directory, ancestor project directories, and `.opencode` config directories. The legacy `config.json` filename is not supported in V2.
